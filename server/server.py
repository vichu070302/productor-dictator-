...
def analyze_freshness(item_key="apple", payload_len=100, filename=""):
    matched_key = item_key if item_key in PRODUCE_DATABASE else "apple"
    for key in PRODUCE_DATABASE:
        if key in (filename + " " + item_key).lower():
            matched_key = key
            break
    ...
    return {
        "success": True,
        "engine": "Google Vision AI & Computer Vision Classifier",
...
class AgriFreshHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/analyze-freshness":
            ...
            filename = ""
            try:
                if post_body.startswith(b"{"):
                    req_data = json.loads(post_body.decode("utf-8", errors="ignore"))
